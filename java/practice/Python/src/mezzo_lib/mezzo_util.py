#! /usr/bin/env python
# -*- coding: utf_8 -*-

import os, sys
import time, datetime


if 'MEZZO_HOME' not in os.environ:
    os.environ['MEZZO_HOME'] = '/home1/irteam/mezzo'

mezzo_home = os.getenv('MEZZO_HOME')
bin_path = os.path.join(mezzo_home, 'bin')
target_path = os.path.join('/home1/irteam/mezzo/', 'out')


def mana_exec(start_time, end_time, re_pattern, re_format, aggr_key_pattern, aggr_op_list, aggr_key_exprs=None, tsv=True):
    """mana를 실행하고 iterator of result rows를 리턴한다."""
    return mana_exec2(start_time, end_time, [(re_pattern, re_format)], aggr_key_pattern, aggr_op_list, aggr_key_exprs, tsv)


def mana_exec2(key_change, start_time, end_time,  progname, re_list, aggr_key_pattern=None, aggr_op_list=None, aggr_key_exprs=None, tsv=True):
    """mana를 실행하고 iterator of result rows를 리턴한다."""
    import csv

    mana_cmd = make_hexec_cmd2(key_change, start_time, end_time, progname, re_list, aggr_key_pattern, aggr_op_list, aggr_key_exprs, tsv=True)
    mana_result = os.popen(mana_cmd, 'rb')

    return create_tsvreader(mana_result)


def make_hexec_regex(re_list):
    reg0 = None
    reg1 = None
    reg2 = None
    reg3 = None
    reg4 = None
    reg5 = None
    reg6 = None
    regex_ = [] 
    for reg in re_list:
        for r in range(len(reg)):
            if(r == 0):
                reg0 = reg[r]
            elif(r == 1):
                reg1 = reg[r]
            elif(r == 2):
                reg2 = reg[r]
            elif(r == 3):
                reg3 = reg[r]
            elif(r == 4):
                reg4 = reg[r]
            elif(r == 5):
                reg5 = reg[r]
            elif(r == 6):
                reg6 = reg[r]
        regex_.append(make_regex_cmd(reg0, reg1, reg2, reg3, reg4,reg5,reg6))
        
    regex =  ' | '.join(regex_)

    return regex


def make_hexec_cmd2(key_change, start_time, end_time,  progname, re_list, aggr_key_pattern=None, aggr_op_list=None,  aggr_key_exprs=None, tsv=True):
    import re

    regex = make_hexec_regex(re_list)
    if(aggr_key_exprs != None):
        aggr_key_exprs = ','.join( [k + ':' + v for k, v in aggr_key_exprs.items()])
    cmd_str_list = []
    cmd_str_list.append( regex )
    cmd_str_list.append( make_aggr_cmd(aggr_key_pattern , aggr_op_list, aggr_key_exprs) )
    hadoop_cmd = make_hexec_cmd( start_time, end_time, cmd_str_list, progname, key_change)
    os.system(hadoop_cmd)

    return make_hexec_cmd2_after_aggr(key_change, start_time, end_time, progname, re_list, aggr_key_pattern, aggr_op_list,  aggr_key_exprs, True)


def make_hexec_cmd2_after_aggr(key_change, start_time, end_time, progname, re_list, aggr_key_pattern=None, aggr_op_list=None, aggr_key_exprs=None, tsv=True):
    import re

    last_aggr_pattern_cmd = ""
    last_aggr_expr_cmd = ""
    last_aggr_op_name = ""
    aggr_last_op_re_first = re.compile('sum\(/([^/]*)/\)')
    aggr_last_op_re = re.compile('\w+=[^<]*<(\w+)>')
    aggr_last_op_re_list = []
    last_aggr_op = []
    if(aggr_op_list != None):
        for last_op in aggr_op_list:
            aggr_last_op_first = aggr_last_op_re_first.search(last_op)
            if(aggr_last_op_first):

                for mo in aggr_last_op_re.finditer(aggr_last_op_first.group(1)):
                    aggr_last_op_re_list.append(mo.group(1).strip() + '=(?P<' + mo.group(1).strip() +'>\d+)')
                temp_ =' '.join(aggr_last_op_re_list)
                last_aggr_op.append('sum(/' + temp_ +'/)')
            else:
                last_aggr_op_name = 'value' 
                last_aggr_op.append('sum(/' + last_aggr_op_name + '=(?P<' + last_aggr_op_name + '>\d+)/)')
    else:
         last_aggr_op = ['sum("1")']

    result = ""
    key_ = []
    last_aggr_expr_cmd_ = []

    if(aggr_key_exprs != None):
        aggr_key_expr_re = re.compile('([^:]*):([^\(]*)\(([^\)]*)\)')
        for mo in aggr_key_expr_re.finditer(aggr_key_exprs):
            if( mo.group(1)[0] == "," ):
                key = mo.group(1)[1:]
            else:
                key = mo.group(1)
            key_.append( key + '=(?P<' + key +'>[^> ]*)' )
            last_aggr_expr_cmd_.append(key + ":echo(" + key + ")")
       
        last_aggr_expr_cmd_.sort(lambda a,b: cmp(a, b))
        key_.sort(lambda a,b: cmp(a, b))
    
        last_aggr_expr_cmd = ','.join(last_aggr_expr_cmd_)
        last_aggr_pattern_cmd = "/" + ' '.join(key_) + "/"  
        result = make_aggr_cmd(last_aggr_pattern_cmd,last_aggr_op,last_aggr_expr_cmd,True)

    else:
        aggr_key_expr_re = re.compile('\(\?P<([^>]*)>[^\)]*\)')
        for mo in aggr_key_expr_re.finditer(aggr_key_pattern):
            key = mo.group(1)
            key_.append(key + '=(?P<' + key +'>[^> ]*)')
        key_.sort(lambda a,b: cmp(a, b)) 
        last_aggr_pattern_cmd = "/" + ' '.join(key_) + "/"
        result = make_aggr_cmd(last_aggr_pattern_cmd, last_aggr_op, aggr_key_exprs,True)

    result = "zcat " + progname + ".gz | " + result

    return result


def create_tsvreader(sequence):
    import csv
    hdrs = sequence.next().strip().split(csv.excel_tab.delimiter)
    return csv.DictReader(sequence, hdrs, dialect='excel-tab')


def make_mana_cmd(start_time, end_time, re_pattern, re_format, aggr_key_pattern, aggr_op_list, aggr_key_exprs=None, tsv=True):
    """mana command string을 만든다."""
    return make_mana_cmd2(start_time, end_time, [(re_pattern, re_format)], aggr_key_pattern , aggr_op_list, aggr_key_exprs, tsv)


def  make_mana_cmd2(start_time, end_time, re_list, aggr_key_pattern=None, aggr_op_list=None,  aggr_key_exprs=None, tsv=True):
    """mana command string을 만든다."""
    draw_expr = 'from ' + start_time.strftime('%Y%m%d_%H') + ' to ' + end_time.strftime('%Y%m%d_%H')

    re_cmds = []
    for re in re_list:
        re_cmd = 'format pattern ' + dbl_quote(re[0]) + ' as ' + dbl_quote(re[1])
        if len(re) == 4:
            re_cmd = 'foreach ' + dbl_quote(re[2]) + ' starts with ' + dbl_quote(re[3]) + ' ' + re_cmd
        re_cmds.append( re_cmd )

    if aggr_key_pattern and aggr_op_list:
        aggr_cmd = ' and calculate ' + ','.join(aggr_op_list)
        if aggr_key_exprs:
            aggr_cmd += ' group by assignment ' + \
                        ','.join( [k + '=' + v for k, v in aggr_key_exprs.items()] ) + \
                        ' from ' + dbl_quote(aggr_key_pattern)
        else:
            aggr_cmd += ' group by ' + dbl_quote(aggr_key_pattern)
    else:
        aggr_cmd = ''

    mana_cmd = 'cd ' + mezzo_home + ';' + \
               'PATH=' + bin_path + ':$PATH; '
    if tsv:
        mana_cmd += 'mana --tsv -g ' + quote(draw_expr + ' ' + ' and '.join(re_cmds) + aggr_cmd)
    else:
        mana_cmd += 'mana -g ' + quote(draw_expr + ' ' + ' and '.join(re_cmds) + aggr_cmd)

    return mana_cmd


def make_hexec_cmd(start_time, end_time , cmds, progname, key_change = False):  
    """command들을 hexec command string을 만든다."""
    cmds = ' | '.join( cmds )
    input_time = start_time.strftime('%Y%m%d_%H') + ' ' + end_time.strftime('%Y%m%d_%H')

    if(key_change):
        cmd = 'hadoop jar /home1/irteam/mezzo/new_scripts/lib/hexec.jar /mezzo/log_hangame_by_loginid/ '  + input_time  + ' ' + progname + ' \'' + cmds + '\' --onlydraw --draw \'-pk li -fo abstype,pluscode,g_ispu,p_ispu\' -g'
    else:
        cmd = 'hadoop jar /home1/irteam/mezzo/new_scripts/lib/hexec.jar /mezzo/log_hangame/ ' + input_time + ' ' + progname + ' \'' + cmds + '\' --onlydraw --draw \'-pk bc,li -fo abstype,pluscode,g_ispu,p_ispu\' -g'

    return cmd  


def make_hexec_cmd_month(start_time, end_time , cmds, progname, key_change = False):
    """command들을 hexec command string을 만든다."""
    cmds = ' | '.join( cmds )
    input_time = start_time.strftime('%Y%m%d_%H') + ' ' + end_time.strftime('%Y%m%d_%H')

    if(key_change):
        cmd = 'hadoop jar /home1/irteam/mezzo/new_scripts/lib/hexec.jar /mezzo/log_hangame_by_loginid/ '  + input_time  + ' ' + progname + ' \'' + cmds + '\' --onlydraw --draw \'-pk li -fo abstype,pluscode,g_ispu,p_ispu\' -g -m 28'
    else:
        cmd = 'hadoop jar /home1/irteam/mezzo/new_scripts/lib/hexec.jar /mezzo/log_hangame/ ' + input_time + ' ' + progname + ' \'' + cmds + '\' --onlydraw --draw \'-pk bc,li -fo abstype,pluscode,g_ispu,p_ispu\' -g -m 28'

    return cmd


def exe_key_change(start_time, end_time, progname, change_type):
    """Key 필드에 있는 내용 중 원하는 내용으로 Key를 체인지 한다"""
    input_time = start_time.strftime('%Y%m%d_%H') + ' ' + end_time.strftime('%Y%m%d_%H')
    os.system('hadoop jar /usr/lib/mezzo/hkchanger.jar /user/irteam/log_hangame/ ' + input_time + ' ' + progname + '_' + change_type + ' ' + change_type)


def make_hg_cmd(cmds):
    """command들을 pipe로 연결한 내용을 command string으로 만들어 준다 """
    pipe_cmds = " | ".join( cmds )
    cmd = 'cd ' + hg_data_home + '; ' + \
          'PATH=' + bin_path + ':$PATH; ' + \
          pipe_cmds
    return cmd


def make_draw_cmd(start_time, end_time):
    """draw command string을 만든다."""
    return 'draw ' + ' '.join( make_draw_args(start_time, end_time) )


def make_draw_args(start_time, end_time):
    """draw command의 argument list를 만든다."""
    return ['-I', 'logs',
            '-s', start_time.strftime('%Y%m%d_%H'),
            '-e', end_time.strftime('%Y%m%d_%H')]


def make_regex_cmd(pattern, format, key_pattern=None, key_format=None, map_pattern=None, split=False, input_file=None):
    """regex command string을 만든다."""
    args = []
    if input_file: args.extend(['-f', dbl_quote(input_file)])
    if key_pattern: args.extend(['-k', dbl_quote(key_pattern)])
    if key_format: args.extend(['-l', dbl_quote(key_format)])
    if split: args.append('--split')
    args.extend([dbl_quote(pattern)])
    if map_pattern: args.extend(['-n', dbl_quote(map_pattern)])
    args.extend([dbl_quote(format)])
    cmd = 'regex ' + ' '.join(args)
    return cmd 


def make_aggr_cmd(key_pattern, op_list, key_expression=None, tsv=False):
    """aggr command string을 만든다."""
    args = ['-k', dbl_quote(key_pattern)]
    if key_expression: args.extend( ['-j', dbl_quote(key_expression)] )
    if tsv: args.append( '--tsv' )
    args.extend( [dbl_quote(op) for op in op_list] )
    cmd = 'aggr ' + ' '.join(args)
    return cmd


def make_checkpoint_pattern(checkpoint):
    """sp parameter가 checkpoint를 포함하는 노드를 찾는 패턴을 만든다."""
    return '<[^>]* sp=[^> ]*' + checkpoint + '[^>]*>'


def make_checkpoints_pattern(checkpoints):
    """여러 개의 checkpoint들을 지나는 노드의 sequence를 찾는 패턴을 만든다."""
    cpps = [make_checkpoint_pattern(cp) for cp in checkpoints]
    p = '(?P<path>' + \
        '(<[^>]*>)*?'.join( cpps ) + \
        ')'
    return p


def parse_timerange( timerange_str ):
    """시작 시각과 끝 시각을 나타내는 문자열의 쌍을 datetime 객체로 변환한다."""
    if len(timerange_str) == 1:
        start_time = mezzo_strptime( timerange_str[0] )
        end_time = start_time + datetime.timedelta(days=1)
    elif len(timerange_str) >= 2:
        start_time = mezzo_strptime( timerange_str[0] )
        end_time = mezzo_strptime( timerange_str[1] )
    else:
        """매일 01시 경에 Cron이 동작한다는 가정하에 24시간을 뺀다"""
        onedayago = datetime.datetime.now() - datetime.timedelta(hours=24)
        start_time = onedayago.replace(hour=00, minute=0, second=0, microsecond=0)
        end_time = start_time + datetime.timedelta(days=1)

    return start_time, end_time


def mezzo_strptime(string):
    """'YYYYMMDD_HH' 형태의 문자열을 datetime 객체로 변환한다."""
    from time import strptime
    return datetime.datetime( *strptime(string, '%Y%m%d_%H')[0:6] )


def quote(string):
    """문자열 앞뒤에 (')를 붙이고 문자열 속의 (') 앞에 (\)를 추가한다."""
    return "'" + string.replace("'", r"\'") + "'"


def dbl_quote(string):
    """문자열 앞뒤에 (")를 붙이고 문자열 속의 (") 앞에 (\)를 추가한다."""
    return '"' + string.replace('"', r'\"') + '"'


def ignore(expr):
    n = expr.split(':')[0]
    return n + ':echo(__all__)'


def combination(seq, mutate):
    if len(seq) == 0: return [[]]

    result = []
    for i in combination(seq[1:], mutate):
        result += [ [seq[0]] + i, [mutate(seq[0])] + i ]
    return result


class StopWatch:
    """Stopwatch that measures time elapsed"""
    def __init__(self, progname=sys.argv[0]):
        self.pname = progname

    def start(self):
        """starts stop watch and returns start time"""
        self.s_time = time.time()
        print >> sys.stderr, self.pname, "started at", time.ctime(self.s_time)

    def stop(self):
        """starts stop watch and print out elapsed time from s_time"""
        e_time = time.time()
        print >> sys.stderr, self.pname, "  ended at", time.ctime(e_time)
        print >> sys.stderr, self.pname, "takes", int(e_time - self.s_time+0.5), "seconds"

