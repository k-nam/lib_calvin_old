...
#outfile = open(os.path.join(target_date_path,'4_3_6_poker_client_usingtime_analysis.tsv'), 'wb')
outfile = open(os.path.join('./4_3_6_poker_clientlog_usingtime_analisys.tsv'),'wb')
...
def main():
    ...
    clsClientUsingTime( start_time, end_time, progname )
    clsPlayTime( start_time, end_time, progname )
    clsWaitingTime( start_time, end_time, progname )
    clsWatchingTime( start_time, end_time, progname )
 
    end = time.ctime()
    print 'End To: ', end


def clsClientUsingTime( start_time, end_time, progname ):

    regex_cmd = r'regex -k "(?P<id><K[^>]*li=[^>]*>)" -l "\g<id>" -s "(?P<sp><[^>]*sp=\"한게임/포커게임/(7포커|하이로우|로우바둑이|맞포커)/클라이언트/경기장_입장\"[^>]*>)" "\g<sp>"'

    regex_pattern = '<[^>]*sp="한게임/포커게임/(7포커|하이로우|로우바둑이|맞포커)/클라이언트/경기장_입장"[^>]*ts=(?P<ts1>"\d*")[^>]*><[^>]*p_(7p|ba|du|hl)_(?P<ground>ground="\d*")[^>]*(?P<p_level>p_level="\d*")[^>]*sp="한게임/포커게임/(?P<cls>7포커|하이로우|로우바둑이|맞포커)/클라이언트/경기장대기실"[^>]*>.*?<[^>]*sp="한게임/포커게임/(7포커|하이로우|로우바둑이|맞포커)/클라이언트/경기장_퇴장" ts=(?P<ts2>"\d*")[^>]*>'

    regex_format = 'cls=\g<cls> \g<ground> \g<p_level> ts1=\g<ts1> ts2=\g<ts2>'
    regex_key_pattern = '<K[^>]*(?P<id>li=[^> ]*)[^>]*>'
    regex_key_format = '\g<id> '

    aggr_key_pattern = '/li="(?P<id>[^> ]*)" cls=(?P<cls>7포커|하이로우|로우바둑이|맞포커) ground="(?P<ground>\d*)" p_level="(?P<p_level>\d*)" ts1="(?P<ts1>\d+)" ts2="(?P<ts2>\d+)"/'
    aggr_key_expr = 'id:echo(id),cls:echo(cls),ground:echo(ground),p_level:echo(p_level),ts:sub(ts2,ts1)'
    aggr_op_list = ['sum(1)']

    aggr_key_pattern1 = '/<k cls=(?P<cls>7포커|하이로우|로우바둑이|맞포커) ground=(?P<ground>\d*) id=(?P<id>[^> ]*) p_level=(?P<p_level>\d*) ts=(?P<ts>\d*)><sum value=(?P<pv>\d*)>/'
    aggr_key_expr1 = 'id:echo(id),cls:echo(cls),ground:echo(ground),p_level:echo(p_level),ts:multiple(ts,pv)'
    aggr_op_list1 = ['sum(1)']

    aggr_key_pattern2 = '/<k cls=(?P<cls>7포커|하이로우|로우바둑이|맞포커) ground=(?P<ground>\d*) id=(?P<id>[^> ]*) p_level=(?P<p_level>\d*)[^>]*>/'
    aggr_key_expr2 = 'id:echo(id),cls:echo(cls),ground:echo(ground),p_level:echo(p_level)'
    aggr_op_list2 = ['sum(/ts=(?P<ts>\d+)/)']

    aggr_key_pattern3 = '/<k cls=(?P<cls>7포커|하이로우|로우바둑이|맞포커) ground=(?P<ground>\d*) id=(?P<id>[^> ]*) p_level=(?P<p_level>\d*)><sum ts=(?P<ts>\d*)>/'
    aggr_key_expr3 = 'cls:echo(cls),request:echo(클라이용시간),ground:echo(ground),p_level:echo(p_level),ts:divide_thirty(ts,0)'
    aggr_op_list3= ['sum(1)']
...
#  포커 게임을 즐긴 시간
def clsPlayTime( start_time, end_time, progname ):

    regex_cmd = r'regex -k "(?P<id><K[^>]*li=[^>]*>)" -l "\g<id>" -s "(?P<sp><[^>]*sp=\"한게임/포커게임/(7포커|하이로우|로우바둑이|맞포커)/클라이언트/경기장대기실\"[^>]*>)" "\g<sp>"'

    regex_pattern = '<[^>]*p_(7p|ba|du|hl)_(?P<ground>ground="\d*") (?P<p_level>p_level="\d*") sp="한게임/포커게임/(7포커|하이로우|로우바둑이|맞포커)/클라이언트/경기장대기실"[^>]*>.*?<[^>]*sp="한게임/포커게임/(?P<cls>7포커|하이로우|로우바둑이|맞포커)/클라이언트/(방_입장|방입장)" ts=(?P<ts1>"\d+")[^>]*>.*?<[^>]*sp="한게임/포커게임/(7포커|하이로우|로우바둑이|맞포커)/클라이언트/(게임방/상단나가기|게임방/하단나가기|방_퇴장|경기장대기실)" ts=(?P<ts2>"\d+")[^>]*>'
    regex_format = 'cls=\g<cls> \g<ground> \g<p_level> ts1=\g<ts1> ts2=\g<ts2>'
    regex_key_pattern = '<K[^>]*(?P<id>li=[^> ]*)[^>]*>'
    regex_key_format = '\g<id> '
    ...
    aggr_key_pattern3 = '/<k cls=(?P<cls>7포커|하이로우|로우바둑이|맞포커) ground=(?P<ground>\d*) id=[^> ]* p_level=(?P<p_level>\d*)><sum ts=(?P<ts>\d+)>/'
    aggr_key_expr3 = 'cls:echo(cls),request:echo(게임이용시간),ground:echo(ground),p_level:echo(p_level),ts:divide_thirty(ts,0)'
    aggr_op_list3 = ['sum(1)']
...
# 최초 경기장 대기실 입장 후 대기실에서 대기한 시간
def clsWaitingTime( start_time, end_time, progname ):
    ...
    aggr_key_pattern3 = '/<k cls=(?P<cls>7포커|하이로우|로우바둑이|맞포커) ground=(?P<ground>\d*) id=[^> ]* p_level=(?P<p_level>\d*)><sum ts=(?P<ts>\d+)>/'
    aggr_key_expr3 = 'cls:echo(cls),request:echo(대기시간),ground:echo(ground),p_level:echo(p_level),ts:divide_thirty(ts,0)'
    aggr_op_list3 = ['sum(1)']
...
# 관전 시간
def clsWatchingTime( start_time, end_time, progname ):
    ...
    aggr_key_pattern3 = '/<k cls=(?P<cls>(7포커|하이로우|로우바둑이|맞포커)) ground=(?P<ground>\d*) id=[^> ]* p_level=(?P<p_level>\d*)><sum ts=(?P<ts>\d+)>/'
    aggr_key_expr3 = 'cls:echo(cls),request:echo(관전시간),ground:echo(ground),p_level:echo(p_level),ts:divide_thirty(ts,0)'
    aggr_op_list3 = ['sum(1)']
...
def write_to_file( _gexec_cmd ):
    ...
    for row in create_tsvreader(os.popen(_gexec_cmd,'rb')):
        ...
        new_row['요청분류'] = row['k.request']
        new_row['구분'] = row['k.cls']
        new_row['경기장'] = row['k.ground']
        new_row['등급'] = row['k.p_level']
        new_row['시간대'] = row['k.ts']
        new_row['PV'] = row['sum.value']
