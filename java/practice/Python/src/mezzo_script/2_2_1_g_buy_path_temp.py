...
"""
아이템 구매의 경우에는 구매 시도 한 번에 여러 개를 구매할 수 있는데,
구매 완료 시도 건수를 산정할 때, 전체 구매 금액 / 개별 아이템 가격 으로 계산되어 있어,
실제 구매 건수와 어느 정도 차이가 발생할 수 있다.
"""


import sys,os
import time
sys.path.append(os.getenv('MEZZO_HOME', '/home1/irteam/mezzo') + '/new_scripts/lib')
...
after_allin_re_cmd = ( '<K[^>]*li=[^>]*>.+(' + allin_node_re + '(?P<path>.+?)\\n)', 
                       r'\g<path>',
                       '(<K[^>]*li=[^>]*>)',
                       r'\1'
                       )
...
find_buy_avataritem_ask_cmd = {
    're_cmds' : [ ( '(?P<ask><[^>]*abstype=[^>]*docref=[^>]*'
                    'g_ispu=[^>]*g_level=[^>]*g_mny_bal=[^>]*'
                    'goodstype="((GOSTOPITEM)|(GOSTOPAVT)|(EDPAVT)|(COMITEM)|(COMAVT)|(GOSTOPVPLUSAVT)|(GOSTOPWPLUSVAT))"[^>]*'
                    'pluscode=[^>]*'
                    'sp=[^> ]*((아바타)|(아이템))/구매시작팝업[^>]*>)',
                    r'\g<ask>',
                    '(<K[^>]*li=[^>]*>)',
...
def find_buy_mship(start_time, end_time, progname): 
    ...
    re_cmds.append( (
         ...
         '((?!<[^>]*sp=[^> ]*회원가입시작[^>]*>)<[^>]*>)*?(?P<done><[^>]*goodslist=(?P<goodslist>[^> ]*)[^>]*sp=[^> ]*회원가입완료[^>]*>)?'
        ,r'\g<ask>\g<done>'
        ,'(<K[^>]*li=[^>]*>)'
        ))
