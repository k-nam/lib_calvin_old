...
target_path = os.path.join('/home1/irteam/hwang/new_script/', 'out')
...
def make_hexec_cmd(start_time, end_time , cmds, progname, key_change = False):  
    ...
    if(key_change):
        cmd = 'hadoop jar ./hexec.jar /mezzo/log_hangame_by_loginid/ '  + input_time  + ' ' + progname + ' \'' + cmds + '\' --onlydraw --draw \'-pk li -fo abstype,pluscode,g_ispu,p_ispu\' -g'
    else:
        cmd = 'hadoop jar ./hexec.jar /mezzo/log_hangame/ ' + input_time + ' ' + progname + ' \'' + cmds + '\' --onlydraw --draw \'-pk bc,li -fo abstype,pluscode,g_ispu,p_ispu\' -g'
...
def make_hexec_cmd_month(start_time, end_time , cmds, progname, key_change = False):
    ...
    if(key_change):
        cmd = 'hadoop jar ./hexec.jar /mezzo/log_hangame_by_loginid/ '  + input_time  + ' ' + progname + ' \'' + cmds + '\' --onlydraw --draw \'-pk li -fo abstype,pluscode,g_ispu,p_ispu\' -g -m 28'
    else:
        cmd = 'hadoop jar ./hexec.jar /mezzo/log_hangame/ ' + input_time + ' ' + progname + ' \'' + cmds + '\' --onlydraw --draw \'-pk bc,li -fo abstype,pluscode,g_ispu,p_ispu\' -g -m 28'
