...
public class DictionaryDao {
	public static List<String> getAutocompleteSuggestions(String prefix, int num) {
		...
		Connection connection = MysqlDbConnector.getConnection("mydata");
		try {
			/* SqlServer code
			...
			String query = "SELECT word, rank FROM dictionary_eng_100k WHERE word LIKE '%" + prefix + "%'" +
					"ORDER BY (CASE WHEN rank IS NULL THEN 1 ELSE 0 END), rank ASC limit " + num + ";";
