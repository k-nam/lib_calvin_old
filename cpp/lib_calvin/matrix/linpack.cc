...
int linpack_main ()
{
	...
	REAL kf;
	static int ipvt[1000],n,i,ntimes,info,lda,ldaa,kflops;
...
/*     compute a residual to verify results.  */ 
	...
	fprintf(stderr," Precision %5d Gflops ; %d Reps \n",kflops / 1000000,NTIMES);
	return 0;
}
...
REAL second()
{
	...
	unsigned lowPart = temp.LowPart;
	unsigned highPart = temp.HighPart;
	double frequency = static_cast<double>(highPart) * pow(2.0, 32)  + lowPart;
	QueryPerformanceCounter(&temp);
	lowPart = temp.LowPart;
	highPart = temp.HighPart;
	double current_time = static_cast<double>(highPart) * pow(2.0, 32)  + lowPart;
