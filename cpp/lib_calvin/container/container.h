...
template <typename T, typename T1>
void insertSingleElement(T *source, size_t size, size_t index, T&& elem, T *target);
...
template <typename Iterator>
bool containerLess(Iterator lhsBegin, Iterator lhsEnd, 
								Iterator rhsBegin, Iterator rhsEnd) 
{	
	while (true) {
		if (lhsBegin == lhsEnd) {
...
template <typename Iterator> 
Iterator advanceIteratorBy(Iterator iterator, ptrdiff_t distance, std::random_access_iterator_tag category) {
	return iterator + distance;
}

template <typename Iterator> 
Iterator advanceIteratorBy(Iterator iterator, ptrdiff_t distance, std::bidirectional_iterator_tag category)  {
	if (distance > 0) {
...
template <typename Iterator> 
Iterator advanceIteratorBy(Iterator iterator, ptrdiff_t distance, std::forward_iterator_tag category)  {
...
template <typename Iterator> 
ptrdiff_t distanceBetweenIterator(Iterator begin, Iterator end, std::random_access_iterator_tag category) {
	return end - begin;
}

template <typename Iterator> 
ptrdiff_t distanceBetweenIterator(Iterator begin, Iterator end, std::bidirectional_iterator_tag category) {
	size_t distance = 0;
	while (begin != end) {
		distance++;
	}
...
template <typename Iterator> 
ptrdiff_t distanceBetweenIterator(Iterator begin, Iterator end, std::forward_iterator_tag category) {
