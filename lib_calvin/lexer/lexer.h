...
class DfaLexerGenerator {
  public:
    ...
    DfaLexerGenerator(): charSize_(static_cast<int>(lib_calvin::getSizeOfCharSet<Alphabet>())) { }
    void clear();
...
lib_calvin_lexer::NfaLexerGenerator<Alphabet>::NfaLexerGenerator(): 
  charSize_(static_cast<int>(lib_calvin::getSizeOfCharSet<Alphabet>())), size_(0) {
}
...
void lib_calvin_lexer::NfaLexerGenerator<Alphabet>::getEpsilonClosure(set<State> const &curStates, 
  ...
  int null = static_cast<int>(lib_calvin::getNullChar<Alphabet>());
