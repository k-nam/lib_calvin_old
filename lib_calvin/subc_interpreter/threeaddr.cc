...
void _main() {
	...
	SP = SP - 300;
	*((int *)M0) = 0;
	STORE( M0 , STACK , FP + (-176) + ZERO , 4 );
	LOAD( GPR1 , STACK , FP + (-176) + ZERO , 4 );
	 GPR0  = 5;
	STORE( GPR0 , STACK , FP + (-132) + GPR1 , 4 );
L42:
	 GPR0  = 4;
	 GPR1  = 0;
	 GPR2  =  GPR0 * GPR1 ;
	STORE( GPR2 , STACK , FP + (-188) + ZERO , 4 );
	LOAD( M0 , STACK , FP + (-188) + ZERO , 4 );
	STORE( M0 , STACK , FP + (-184) + ZERO , 4 );
...
L44:
	 ...
	 GPR1  = 1;
	 GPR2  =  GPR0 > GPR1 ;
	STORE( GPR2 , STACK , FP + (-208) + ZERO , 4 );
	LOAD( GPR0 , STACK , FP + (-208) + ZERO , 4 );
	IF ( GPR0 , L49 );
	GOTO (L48);
L49:
	 GPR0  = 3;
	 GPR1  = 2;
	 GPR2  =  GPR0 > GPR1 ;
	STORE( GPR2 , STACK , FP + (-212) + ZERO , 4 );
	LOAD( GPR0 , STACK , FP + (-212) + ZERO , 4 );
	IF ( GPR0 , L46 );
	GOTO (L48);
L46:
	 GPR0  = 4;
	...
	LOAD( M0 , STACK , FP + (-224) + ZERO , 4 );
	STORE( M0 , STACK , FP + (-220) + ZERO , 4 );
	LOAD( GPR1 , STACK , FP + (-220) + ZERO , 4 );
	 GPR0  = 4;
	STORE( GPR0 , STACK , FP + (-24) + GPR1 , 4 );
L47:
	GOTO (L45);
...
L45:
	...
	LOAD( GPR1 , STACK , FP + (-236) + ZERO , 4 );
	 GPR0  = 100;
	STORE( GPR0 , STACK , FP + (-128) + GPR1 , 4 );
L50:
	GOTO (L54);
L52:
	 GPR0  = 8;
	...
	LOAD( GPR0 , STACK , FP + (-244) + ZERO , 4 );
	 GPR1  = 4;
	 GPR2  =  GPR0 + GPR1 ;
	STORE( GPR2 , STACK , FP + (-260) + ZERO , 4 );
	LOAD( GPR1 , STACK , FP + (-260) + ZERO , 4 );
	 GPR0  = 11;
	STORE( GPR0 , STACK , FP + (-128) + GPR1 , 4 );
L53:
	GOTO (L51);
...
L55:
	...
	LOAD( M0 , STACK , FP + (-272) + ZERO , 4 );
	STORE( M0 , STACK , FP + (-268) + ZERO , 4 );
