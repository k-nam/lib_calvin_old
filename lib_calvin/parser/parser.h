#ifndef LIB_CALVIN__PARSER__PARSER_H
#define LIB_CALVIN__PARSER__PARSER_H
...
namespace lib_calvin_parser {
	...
	// Types for parsing 
	enum class Action : int { // Action::Error is just empty slot
		Shift = 0,
		Reduce = 1,
		Accept = 2,
		Error = 3
	};
	enum class Association : int { // type of associativity
		Left = 1,
		Right = 2,
	...
	public:
		...
		SlrParserGenerator(ContextFreeLanguage const &inLang) :
			language_(inLang), size_(0) { }
	...
	protected:
		...
		// ..terminal
		int getPrecedence(int terminal) const { // return -1 when not set
	...
	public:
		...
		struct LR1Item :
			public SlrParserGenerator::LR0Item {
			// Default dot position is 0
			LR1Item(Production const &inPro, int inLookAhead) :
	...
	public:
		typedef ContextFreeLanguage::Production Production;
		LalrParserGenerator(ContextFreeLanguage const &inLang) :
			Lr1ParserGenerator(inLang) { }
...
} // end namespace lib_calvin_parser
