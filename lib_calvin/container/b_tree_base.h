...
namespace lib_calvin_container
{
	...
	private:
		...
		public:
			...
			template <typename T1> void insertElement(int64_t index, T1 &&elem);
	...
	private:
		...
		template <typename T1> void insertValue(Node *node, T1 &&elem, int64_t indexToInsert);
	...
	template <typename T, typename K, typename Comp, typename ExtractKey>
	B_TREE_BASE<T, K, Comp, ExtractKey>::IteratorImpl::IteratorImpl(
		B_TREE_BASE<T, K, Comp, ExtractKey> const *tree, Node *node, int64_t index) :
...
#define ADD_TO_NODE_ROUTINE_LEAF(ARG2, ARG4, ARG6, ARG7, ARG8) \
	...
	template <typename T, typename K, typename Comp, typename ExtractKey>
	void
		B_TREE_BASE<T, K, Comp, ExtractKey>::addToLeafNodeFromLeft(Node *node, Node *brotherNode, int64_t parentIndex) {
		ADD_TO_NODE_ROUTINE_LEAF(-1, getLastElement, eraseLastElement, insertFirstElement, brotherNode)
	}

	template <typename T, typename K, typename Comp, typename ExtractKey>
	void
		B_TREE_BASE<T, K, Comp, ExtractKey>::addToLeafNodeFromRight(Node *node, Node *brotherNode, int64_t parentIndex) {
...
#define ADD_TO_NODE_ROUTINE_INTERNAL(ARG1, ARG2, ARG3, ARG4, ARG5) \
	...
	template <typename T, typename K, typename Comp, typename ExtractKey>
	void
		B_TREE_BASE<T, K, Comp, ExtractKey>::addToInternalNodeFromLeft(Node *node, Node *brotherNode, int64_t parentIndex) {
		ADD_TO_NODE_ROUTINE_INTERNAL(insertFirstElementAndChild, -1, getLastChild, getLastElement, eraseLastElementAndChild)
	}

	template <typename T, typename K, typename Comp, typename ExtractKey>
	void
		B_TREE_BASE<T, K, Comp, ExtractKey>::addToInternalNodeFromRight(Node *node, Node *brotherNode, int64_t parentIndex) {
