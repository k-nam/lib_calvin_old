...
namespace lib_calvin_container
{
	...
	protected:
		...
		template <typename T1> std::pair<Node *, bool> insert_impl(T1 &&key);
...
namespace lib_calvin_container // open for definitions
{
	...
	template <typename T, typename K, typename Comp, typename ExtractKey>
	bool
		BIN_TREE_BASE<T, K, Comp, ExtractKey>::Node::hasLeftChild() const {
		return hasChild(Direction::Left);
	}

	template <typename T, typename K, typename Comp, typename ExtractKey>
	bool
		BIN_TREE_BASE<T, K, Comp, ExtractKey>::Node::hasRightChild() const {
		return hasChild(Direction::Right);
	}

	template <typename T, typename K, typename Comp, typename ExtractKey>
	bool
		BIN_TREE_BASE<T, K, Comp, ExtractKey>::Node::hasChild(Direction direction) const {
	...
	template <typename T, typename K, typename Comp, typename ExtractKey>
	typename BIN_TREE_BASE<T, K, Comp, ExtractKey>::IteratorImpl &
		BIN_TREE_BASE<T, K, Comp, ExtractKey>::IteratorImpl::operator++() { // prefix
	...
	typename BIN_TREE_BASE<T, K, Comp, ExtractKey>::IteratorImpl const
		BIN_TREE_BASE<T, K, Comp, ExtractKey>::IteratorImpl::operator++(int) { // suffix
	...
	template <typename T, typename K, typename Comp, typename ExtractKey>
	bool
		BIN_TREE_BASE<T, K, Comp, ExtractKey>::IteratorImpl::operator==(IteratorImpl const &rhs) const {
	...
	template <typename T, typename K, typename Comp, typename ExtractKey>
	BIN_TREE_BASE<T, K, Comp, ExtractKey>::BIN_TREE_BASE(BIN_TREE_BASE<T, K, Comp, ExtractKey> const &rhs) :
		size_(0), root_(nullptr), end_(nullptr) {
	...
	template <typename T, typename K, typename Comp, typename ExtractKey>
	BIN_TREE_BASE<T, K, Comp, ExtractKey>::BIN_TREE_BASE(BIN_TREE_BASE<T, K, Comp, ExtractKey> &&rhs) :
		size_(0), root_(nullptr), end_(nullptr) {
	...
	std::pair<typename BIN_TREE_BASE<T, K, Comp, ExtractKey>::Node *, int>
		...
		if (is_null<T, K, Comp, ExtractKey>(successor) || Comp()(key, ExtractKey()(successor->getKey()))) { // not exists, so insert
	...
	typename BIN_TREE_BASE<T, K, Comp, ExtractKey>::Node *
		...
		if (nodeToDelete->hasLeftChild() != nodeToDelete->hasRightChild()) { // only one child exist
			BIN_TREE_BASE<T, K, Comp, ExtractKey>::Node *child = nodeToDelete->hasLeftChild() ?
	...
	template <typename T, typename K, typename Comp, typename ExtractKey>
	void
		BIN_TREE_BASE<T, K, Comp, ExtractKey>::binTreeNodeCopy(BIN_TREE_BASE<T, K, Comp, ExtractKey>::Node *srcNode, BIN_TREE_BASE<T, K, Comp, ExtractKey>::Node *&targetNode)
	{
#ifdef USE_HEAD_NODE
		BIN_TREE_BASE<T, K, Comp, ExtractKey>::Node *nullNode = end_;
#else
	...
	template <typename T, typename K, typename Comp, typename ExtractKey>
	void
		BIN_TREE_BASE<T, K, Comp, ExtractKey>::prepareNode(BIN_TREE_BASE<T, K, Comp, ExtractKey>::Node *newNode) const {
	...
	template <typename T, typename K, typename Comp, typename ExtractKey>
	bool
		is_null(typename BIN_TREE_BASE<T, K, Comp, ExtractKey>::Node const *node) {
#ifdef USE_HEAD_NODE
