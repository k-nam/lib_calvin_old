...
def solve(data, room_size, carpet_size, num_colors):
	...
	while (len(remaining_colors) > 0):
		for color in remaining_colors:
			...
			if (square_position):
				...
				break
	return "\n".join([str(row[0]) + ' ' + str(row[1][1] + 1) + ' ' + str(row[1][0] + 1) for row in result])
...
def main(input):
	...
	return solve(data, room_size, carpet_size, num_colors)
