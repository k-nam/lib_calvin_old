...
def is_inside(data, h, w, position):
...
def get_succesors(data, h, w, previous):
	def filter_succesors(succesors):
		return [x for x in succesors if is_inside(data, h, w, x[0])]
...
def solve(data, h, w):
	...
	while (len(heap) > 0):
		...
		if not is_finished(elem[1]):
			succesors = get_succesors(data, h, w, elem[1])
