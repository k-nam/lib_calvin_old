                         ...
                         std::vector<range<Value_t *> > &v_output, Compare comp)
{
    ...
    auto nrange = v_input.size();
    auto pos_ini = 0;
    while (pos_ini < v_input.size())
    {
		auto nmerge = (nrange + 3) >> 2;
		auto nelem = (nrange + nmerge - 1) / nmerge;
        range1_t rz = uninit_full_merge4(dest, &v_input[pos_ini], static_cast<uint32_t>(nelem), comp);
        v_output.emplace_back(rz);
        dest.first = rz.last;
        pos_ini += static_cast<int>(nelem);
        nrange -= nelem;
    };
    return;
};
